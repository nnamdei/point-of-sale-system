...
class DeskController extends Controller
{
    ...
    public function scanBarcode(Request $request){
        ...
        if($barcode == null){
            return redirect()->back()->with('error', '<strong>FAILED!</strong> Could not verify barcode');
        }
        ...
        if($barcode->isGenerated()){ //if the barcode was generated by my BarcodeTrait, decrypt it!
            ...
            if($product == null){
                return redirect()->back()->with('error', '<strong>FAILED!</strong> Product not found');
            }
                ...
                if($product->variants->count() > 0){
                    foreach($product->variants as $variant){
                        for($i=1; $i<= count($content); $i++){ //pick out the variant(s)
                            if(!isset($content[$i])){
                                return redirect()->route('desk.cart')->with('error','<strong>FAILED! </strong>'.$variant->variable.' of '.$product->name.' could be read'); 
                            }
        ...
        if($product !== null && $cart !== null){
            if(!empty($cart['infeasibility'])){
                return redirect()->back()->withErrors($cart['infeasibility']);
            }
            Cart::add($cart['id'],$cart['name'],$cart['qty'],$cart['price'],$cart['opts'])->associate('App\Product');
            return redirect()->route('desk.cart')->with('success',$product->name.' added to cart');    
        }
        return redirect()->back()->with('error', '<strong>FAILED! </strong>');    
    }
