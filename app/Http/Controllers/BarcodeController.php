...
class BarcodeController extends Controller
{
    ...
    public function __construct(){
       // $this->middleware('premium');
        $this->middleware('manager')->except(['scanProduct']);
    }
    ...
    public function scanProduct(Request $request){
        ...
        if(session('scanner_action') =='capture_barcode'){
            session()->put('captured_barcode', $request->content);
            return redirect()->back()->with('success', 'Barcode captured, you can now attach it to any product');
        }
        ...
        if($barcode == null){
        /**
         * The barcode generator C39+ adds suffix character to generated barcode, this is to strip it off, and convert the contents to lower case, then check again
         */
        $sanitized_content = strtolower(substr($request->content,0,strlen($request->content)-1));
        $barcode = Barcode::where('barcode_content',$sanitized_content)->first();
            if($barcode == null){
        ...
        if(session('scanner_action') =='add_to_cart'){
            $cart = null;
            $quantity = 1;

            if($barcode->isGenerated()){ //if the barcode was generated by my BarcodeTrait, decrypt it!
                $content = explode('$',$sanitized_content); //seperate the id and attribute
                $product = $barcode->product;
                $option = array();
                    ...
                    if($product->variants->count() > 0){
                        foreach($product->variants as $variant){
                            for($i=1; $i< count($content); $i++){ //pick out the variant(s)
                                if(!isset($content[$i])){
                                    break;
                                    return redirect()->route('desk.cart')->with('barcode_error','<strong>FAILED! </strong>'.$variant->variable.' of '.$product->name.' could not be read'); 
                                }
    ...
    public function clearCapturedBarcode(){
        Session::remove('captured_barcode');
        return redirect()->back()->with('success', 'Captured barcode cleared');
    }
        ...
        public function generateRandomBarcode(){
            $serial = substr(Auth::user()->shop->name,0,1).rand(100000,999999);
            ...
            $pdf = PDF::loadView('barcode', ['barcode' => $barcode, 'serial' => $serial]);
            return $pdf->stream('barcode-'.$serial.'.pdf');
         }
