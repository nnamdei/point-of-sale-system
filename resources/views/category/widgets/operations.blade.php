<a class="btn btn-info btn-sm m-1" title="edit category {{$category->name}}" href="{{route('categories.edit', ['id'  => $category->id])}}"><i class="fa fa-pen"></i> edit</a>
<button class="btn btn-danger btn-sm m-1" data-toggle="collapse" data-target="#discard-category-{{$category->id}}-collapse" title="trash {{$category->name}}"><i class="fa fa-times"></i> Trash all products</button>            
